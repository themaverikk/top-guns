import { Box, Button, FormControl, FormHelperText, GridList, IconButton } from '@material-ui/core';
...
const Skills = () => {
    ...
    const toggleSkill = selectedSkillIndex => {
        ...
        const isSkillSelected = applicationState.skills[selectedSkillIndex].isSelected;
        const updatedSkill = { ...applicationState.skills[selectedSkillIndex], isSelected: !isSkillSelected };
    ...
    const handleSubmit = event => {
        ...
        const selectedSkills = applicationState.skills.filter(skill => skill.isSelected);
                        ...
                        <Box component="div" display="block" className={classes.root}>
                            <GridList className={classes.gridList} cols={4}>
                                {applicationState.skills.map((skill, index) => (
                                    <SkillTile key={skill.id} skill={skill} toggleSkill={() => toggleSkill(index)} />
                                ))}
                            </GridList>
                        </Box>
                        {error && <FormHelperText>Please select at least one skill</FormHelperText>}
